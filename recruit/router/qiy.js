...
Router.post('/fqiy', (req, res) => {
    ...
    let total = 0;
    qiyModel.find(job ? { $or: [{ job: { $regex: job } }, { address: { $regex: job } }, { ctype: { $regex: job } }] } : null)
        .then((rrr) => {
            total = rrr.length;
            qiyModel.find(job ? { $or: [{ job: { $regex: job } }, { address: { $regex: job } }, { ctype: { $regex: job } }] } : null).limit(pagesize).skip((target - 1) * pagesize)
                .then((data) => {
                    let array = { total: total, studerlist: data }
                    res.send(utli.sendData(0, 200, array))
                })
        })
        .catch((err) => {
...
Router.post('/val', (req, res) => {
    let job = req.body.va;
    qiyModel.find({ $or: [{ job: { $regex: job } }, { address: { $regex: job } }, { ctype: { $regex: job } }] })
        .then((data) => {
            // console.log(data)
